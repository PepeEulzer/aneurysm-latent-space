...
# --- Config ---
model_type = "pointnet_vae"
loss_type = "pointwise_distance_kld" 
...
for out_dim in out_dims:
    for z_size in z_sizes:
        print(f"\n{'='*50}\nTraining {model_type} | out_dim: {out_dim} | z_size: {z_size} | beta: {beta}\n{'='*50}")
        ...
        min_val_loss = float('inf') # FIXED: Moved OUTSIDE the epoch loop
        epochs_no_improve = 0
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            total_train_loss, total_pwd_loss, total_kl_loss = 0.0, 0.0, 0.0
            ...
            for i, (point_data, _) in enumerate(train_loader, 1):
                ...
                # FIXED: Reparameterization Trick (Unique noise per batch item)
                sigmas = torch.exp(logvar_gold * 0.5)
                if sigma_scaling > 0.0:
                    epsilon = torch.randn_like(sigmas) # FIXED: Was torch.randn(z_size)
                ...
                # Losses
                pointwise_distance_loss = torch.mean(torch.norm(X_rec - X, p=2, dim=2)) 
                
                mu2 = torch.pow(mu_gold, 2) 
                sigmas2 = torch.exp(logvar_gold)
                kl_divergence_loss = beta * torch.sum(mu2 + sigmas2 - logvar_gold - 1)

                loss = pointwise_distance_loss + kl_divergence_loss 
                ...
                total_pwd_loss += pointwise_distance_loss.item()
            ...
            with torch.no_grad():
                for point_data, _ in val_loader:
                    ...
                    # For validation, we typically use the mean directly (no noise) to measure true reconstruction capability
                    z_val = mu_val
                    ...
                    val_kl_loss = beta * torch.sum(torch.pow(mu_val, 2) + torch.exp(logvar_val) - logvar_val - 1)
                    
                    val_loss = val_pwd_loss + val_kl_loss
            ...
            if epoch % 10 == 0 or epoch == 1:
                log_str = (f'Epoch: {epoch:04d} | Train Loss: {avg_train_loss:.4f} (PWD: {total_pwd_loss/len(train_loader):.4f}, '
                           f'KL: {total_kl_loss/len(train_loader):.4f}) | Val Loss: {avg_val_loss:.4f} | Time: {datetime.now() - start_epoch_time}')
                print(log_str)
